...
namespace Pyramid
{
    namespace Scene
    {
        ...
        SceneManager::SceneManager()
            ...
            , m_octreeMaxDepth(8)
            , m_octreeSize(1000.0f, 1000.0f, 1000.0f)
            , m_octreeCenter(Math::Vec3::Zero)
            , m_lodEnabled(true)
        ...
        QueryResult SceneManager::QueryScene(const QueryParams& params)
        {
            ...
            else
            {
                ...
                for (const auto& obj : allObjects)
                {
                    ...
                    if (obj && obj->position.DistanceTo(params.position) <= params.radius)
                    {
        ...
        std::shared_ptr<RenderObject> SceneManager::GetNearestObject(const Math::Vec3& position)
        {
            ...
            for (const auto& obj : allObjects)
            {
                if (obj)
                {
                    f32 distance = obj->position.DistanceTo(position);
        ...
        f32 SceneManager::CalculateLOD(const std::shared_ptr<RenderObject>& object, const Camera& camera)
        {
            ...
            f32 distance = object->position.DistanceTo(camera.GetPosition());
            
            // Simple LOD calculation based on distance
            if (distance < 10.0f) return 1.0f;      // High detail
            else if (distance < 50.0f) return 0.5f; // Medium detail
            else return 0.25f;                      // Low detail
        }
