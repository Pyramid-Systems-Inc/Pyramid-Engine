...
namespace Pyramid
{
    namespace Util
    {
        ...
        private:
            ...
            /**
             * @brief Decode JPEG image data using our custom components
             *
             * @param decoder JPEG decoder state with parsed headers and tables
             * @param imageData Pointer to compressed image data
             * @param imageDataSize Size of compressed image data
             * @param result Output ImageData structure to fill
             * @return true if successful
             */
            static bool DecodeJPEGImageData(const JPEGDecoder &decoder,
                                            const uint8_t *imageData,
                                            size_t imageDataSize,
                                            ImageData &result);

            /**
             * @brief Create a test pattern to verify JPEG integration
             *
             * @param result ImageData structure to fill with test pattern
             * @param width Image width
             * @param height Image height
             */
            static void CreateTestPattern(ImageData &result, int width, int height);
