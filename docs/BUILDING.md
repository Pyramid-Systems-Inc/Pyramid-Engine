...
CTest registers 26 executables:
...
- `Graphics.SceneTransforms`
- `Graphics.CameraFrustum`
- `Graphics.OctreeUpdates`
- `Graphics.OctreeQueries`
- `Graphics.NearestQueries`
- `Graphics.OctreeConfiguration`
- `Graphics.OctreeCompaction`
- `Graphics.RenderObjectBounds`
- `Graphics.MeshResource`
- `Graphics.MeshCache`
- `Graphics.ShaderCache`
- `Graphics.TextureCache`
- `Graphics.MaterialResource`
- `Graphics.MaterialCache`
- `Graphics.ResourceRegistry`
- `Graphics.ResourceHandles`

The API test takes addresses of public factory, scene-management, camera-frustum, bounds, and resize methods so missing definitions fail at link time. The OpenGL diagnostics test verifies readable error, source, type, and severity mappings without requiring a graphics context. The platform test verifies callback delivery, replacement, detachment, window state, and zero-area handling without opening a native window. The viewport test verifies perspective and orthographic projection resizing. `Graphics.CameraFrustum` verifies normalized plane extraction, negative-Z camera orientation, point/sphere/AABB classification, transformed render-object bounds, scene-manager behavior, and octree rejection. `Graphics.OctreeUpdates` verifies incremental branch relocation, bounds-only changes, additions, removals, duplicate snapshot handling, and stable-scene no-op synchronization. `Graphics.OctreeQueries` verifies complete-AABB point/sphere/box/ray intersections, root-overflow objects, invalid inputs, uniqueness, nearest-first ray ordering, and linear/octree parity. `Graphics.NearestQueries` verifies closest-point AABB distance, bounds-aware nearest/K-nearest ordering, zero and oversized K values, root-overflow objects, hidden gameplay objects, and octree/linear parity. `Graphics.OctreeConfiguration` verifies atomic root-bounds/depth/capacity rebuilds, tracked-object preservation, invalid and non-finite input rejection, zero-capacity normalization, and combined configuration updates. `Graphics.OctreeCompaction` verifies automatic branch collapse after removals and movement, batched synchronization statistics, explicit no-op compaction, survivor queries, and health-metric consistency. `Graphics.RenderObjectBounds` verifies geometry-derived and manual world bounds. `Graphics.MeshResource` verifies immutable validated mesh ownership and indexed/non-indexed draw metadata. `Graphics.MeshCache` verifies stable IDs, exact-content sharing across aliases, one-time GPU upload, conflict rejection, strong residency, unused collection, explicit eviction, and external-owner lifetime. `Graphics.ShaderCache` verifies stable source IDs, compile-once reuse, stage validation, immutable program forwarding, alias conflicts, transactional replacement, failure preservation, replacement reuse, and lifetime statistics. `Graphics.TextureCache` verifies stable texture identity, deduplication, color-space separation, transactional reload, and residency. `Graphics.MaterialResource` verifies immutable material identity, validation, render-state application, texture bindings, typed uniforms, and command-buffer ordering. `Graphics.MaterialCache` verifies exact-content sharing across aliases, conflict rejection, transactional replacement, failed-replacement preservation, resident-content reuse, external-owner lifetime, residency statistics, collection, and eviction. `Graphics.ResourceRegistry` verifies game-level cache ownership semantics, dependency-safe collection/clearing, aggregate statistics, retained material dependencies, and external-owner lifetime. `Graphics.ResourceHandles` verifies typed ID/generation identity, acquisition/resolution, stale-handle rejection after direct cache mutation or replacement, non-owning collection behavior, scene-object integration, and registry clearing. `Graphics.ResourceManifest` verifies deterministic versioned serialization, typed handle reconstruction, transactional parser rollback, malformed-entry diagnostics, and missing/stale registry restoration reports. The framebuffer test verifies specification structure, multisample consistency, and state preservation for rejected zero-area resizes without creating a graphics context. The texture test uses a fake OpenGL backend to verify sRGB upload formats, unpack alignment, non-mip filter safety, failed-reload preservation, and full-data updates. The scene-transform test verifies multi-generation TRS composition, recursive cache invalidation, reparenting, detachment, cycle rejection, and parent-destruction behavior without a graphics context. JPEG utility tests decode standards-valid baseline and progressive fixtures.
