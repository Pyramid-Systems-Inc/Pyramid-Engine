...
The platform layer may expose bytes from fonts legally installed on the host. The Win32 implementation uses GDI only to select and extract a TrueType face; `Pyramid::Font` still performs parsing, validation, rasterization, deterministic caching, and processed-asset publication. `BasicGame` tries Segoe UI, Tahoma, and Arial before falling back to the bundled Ruqoom-owned reference faces. This improves production UI appearance without bundling another party's font or introducing font middleware.
