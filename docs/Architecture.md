...
The camera extracts six normalized inward-facing frustum planes from its view-projection matrix. Camera point, sphere, and AABB tests share those planes. `Mesh` is the authoritative geometry resource: it owns vertex/index buffers and their vertex array, validates layout/topology/counts, freezes local bounds at creation, and stores stable asset/content identifiers. `MeshCache` is graphics-device-bound and indexes strong resident resources by content fingerprint while mapping caller-defined stable aliases to the same content. Exact geometry is uploaded once even when requested under multiple identifiers; conflicting alias/content pairs are rejected. Explicit eviction or unused collection releases cache ownership without invalidating external shared owners. `RenderObject` can reference mesh/material resources directly for compatibility or through typed registry handles. Handle-backed objects retain immutable mesh bounds for culling but not the mesh owner; render passes resolve the current matching generation through their registered `ResourceRegistry`, and stale handles skip submission. Explicit manual bounds take precedence and the unit cube remains the fallback. All eight local-bound corners are transformed into a world-space AABB, and both linear scene visibility and octree queries use that same bound. Objects spanning octree child boundaries remain in the parent node, allowing child-node rejection without hiding intersecting geometry.

The octree supports insertion, removal, incremental synchronization, bottom-up compaction, explicit rebuilding, atomic configuration, bounds-aware nearest-neighbor queries, health metrics, and public spatial helpers. Nearest traversal visits children by the minimum point-to-node distance and prunes branches once their lower bound exceeds the farthest retained candidate. It tracks the last world AABB for each object. During the normal `SceneManager::Update()` path, new and removed scene objects are reconciled and only objects with changed bounds are removed and reinserted; stable objects do not mutate the tree. After movement or removal, one compaction pass collapses empty descendant branches and promotes a subtree into its parent when the complete object set fits that parent's configured capacity. Configuration changes validate the complete root bounds/depth/capacity request, build a replacement tree off to the side, reinsert all tracked objects, and only then swap live state. Invalid settings or replacement-construction failures preserve the previous tree. Health statistics distinguish configured depth from occupied depth and report node/leaf occupancy, empty leaves, tracked versus stored objects, and approximate memory usage. Low-level OpenGL vertex buffers can still derive transient bounds while a mesh is being built, but `Mesh::Create()` records the validated AABB as immutable resource metadata and does not expose mutable buffers through `RenderObject`. The future asset importer should produce these mesh specifications and stable IDs directly; runtime content deduplication and cache residency are already implemented. Occlusion culling remains a placeholder and is disabled by default.
