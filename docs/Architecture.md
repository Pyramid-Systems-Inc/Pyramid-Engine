...
`Pyramid::Text` consumes baked font data and owns strict UTF-8 conversion, fallback accounting, kerning-aware line measurement, tab expansion, character/word wrapping, horizontal alignment, renderer-neutral glyph-quad generation, and the code-point-indexed `TextBuffer` editing model. It retains the embedded ASCII atlas as an emergency diagnostic fallback. Bidirectional layout, Arabic/complex-script shaping, fallback font families, and full IME composition remain later milestones.
