...
`UIRenderer` is the engine graphics adapter. It owns the UI shader, linearly sampled grayscale font-atlas texture, dynamic buffers, texture-ID registration, alpha-blended batching, the explicit final-surface framebuffer/viewport, conservative DPI-scaled scissor rectangles, and deterministic baseline-state restoration. The owned reference fonts are baked at 48 pixels and scaled down to stable logical UI sizes so CPU supersampling and GPU linear sampling cooperate instead of exposing source-grid stair stepping. Initialization is transactional: failures remove only shader/texture aliases and content introduced by that attempt while preserving unrelated or previously resident cache resources. `Pyramid::UI` never includes GLAD or issues graphics calls.
