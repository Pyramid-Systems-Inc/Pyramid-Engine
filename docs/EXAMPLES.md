...
- exact installed Windows families selected per script—Segoe UI for Latin, Cairo when installed or Tahoma for Arabic, and Segoe UI Symbol for icons—plus required-glyph/capacity gates, script-specific processed caches, Ruqoom-owned `Pyramid Sans` and `Pyramid Arabic` deterministic 64-pixel SDF fallbacks, semantic typography roles, an ordered fallback family, grapheme/bidirectional/Arabic-aware layout, bilingual validation labels, executable-relative runtime resolution, and embedded-font fallback when all production sources are unavailable;
